...
- **Frontend:** только функциональные компоненты. Вся видимая пользователю строка — через
  ...
  как минимум добавляй ключ в `en` (и `ru`), остальные локали можно добить отдельной задачей позже.
...
- Локализация: при добавлении/изменении UI-строки — обязательно ключ в `en`, `ru`; остальные
  16 локалей могут временно отставать (это не блокирует мерж) — довести их до конца отдельной
  задачей, зафиксированной в GitHub Issue, если накопилось заметное расхождение.
