...
Проект **уже построен и работает** — приоритет не «сгенерировать с нуля», а поддерживать код на
уровне, с которого не стыдно брать пример: держать зелёный билд, не давать сервисам/страницам
разрастаться обратно за пределы устоявшегося стиля, регулярно подтягивать зависимости и закрывать
security-алерты (см. [`/maintain`](.claude/commands/maintain.md)).

Главная сессия Claude делает большинство работы **сама**. Специалисты-агенты (`.claude/agents/*`)
вызываются точечно — для архитектурных решений, чувствительных к безопасности мест, тестирования,
CI/CD/Docker и read-only проверки сборки — не на каждую мелкую правку. Перед работой прочитай:
...
- **Процесс команды** (роли агентов, правила вызова) — в `.claude/`.
...
Область действия правила — страницы со списком/состоянием (загрузка/ошибка/пусто/данные из
«Architecture» выше). Простые формы-страницы (`LoginPage`, `RegisterPage`, `ForgotPasswordPage`,
`ResetPasswordPage`) — второй, тоже устоявшийся паттерн: одноразовый gRPC-вызов прямо в обработчике
`react-hook-form`-сабмита, без отдельного хука ради одного вызова. Не переноси их на паттерн
page+hook по инерции — это не нарушение, а другой класс страниц.
...
- **Frontend:** только функциональные компоненты. Вся видимая пользователю строка — через
  ...
  как минимум добавляй ключ в `en` (и `ru`), остальные 16 локалей может добить `translator`
  (haiku, см. `.claude/agents/translator.md`) по точному списку ключей.
...
- Один файл — один (экспортируемый) компонент, плюс тривиальный приватный под-компонент, если
  это реально упрощает файл. Не сваливай несколько компонентов в один файл — `ui-developer`
  следит за этим и разносит по файлам, `repo-scout` собирает сигнал для `/maintain`.
...
- Плановый проход поддержки (зависимости + security-алерты + рефакторинг + технологический радар)
  — команда [`/maintain`](.claude/commands/maintain.md), рассчитана на регулярный повтор, не
  только на разовый запрос.
...
- Локализация: при добавлении/изменении UI-строки — обязательно ключ в `en`, `ru`; остальные
  16 локалей могут временно отставать (это не блокирует мерж) — довести их до конца через
  `translator` (см. `.claude/agents/translator.md`), если накопилось заметное расхождение, не
  оставлять бесконтрольно копиться.
